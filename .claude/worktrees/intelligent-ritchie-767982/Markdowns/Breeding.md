...
Status:          IN PROGRESS - Phase 2/3 partial (Patch 2)

CHANGELOG:
- 1.1: Added Phase 1-5 refactor plan for actor-agnostic architecture.
	   Marked completed items. Updated file structure.
- 1.2: Patch 2 — B7 fixed (HerdManager.next_day accepts any ActorData kid).
	   ActorCard now generic via `actor_data: ActorData` + `goat_data` alias.
	   Ranch.gd typed members generalized to ActorData; sort uses
	   `get_display_name()`. GoblinData / FarmerData / ElementalData
	   created with their own `create_offspring()`. Non-goat actors
	   (GoblinMinion, FarmerActor, FireActor, WaterActor) gained typed
	   `_data` accessors; hardcoded ability-score blocks now gated on
	   `_data == null` to avoid clobbering resource-driven stats.
	   GoatRenderer rename intentionally deferred (live .tscn refs).
	   ProgressionComponent still touches GoatData-only fields
	   (`age_days`, `stamina_max`, `stamina_current`); ensure
	   day-advance for non-goat herd members in the next pass.
...
- [x] ✅ Refactor ActorCard to use `ActorData` generic (Patch 2 — `actor_data: ActorData` is primary API; `goat_data: GoatData` kept as typed alias for legacy callers in `Ranch.gd`)
- [ ] ⬜ Rename `GoatRenderer` → `ActorCardRenderer` (deferred — three live `.tscn` references; see AGENT_2_PATCH2_LOG.md §10)
- [x] ✅ Update Ranch.gd for generic actor selection (Patch 2 — `selected_doe/buck: ActorData`, sort via `get_display_name()`, handler renamed `_on_actor_selected`)
- [x] ✅ Fix B7: `HerdManager.next_day()` accepts any ActorData kid, not just GoatData (Patch 2)
- [x] ✅ Add `get_display_name()` / `get_info_line()` / `set_display_name()` virtuals on ActorData (Patch 2)
- [x] ✅ Lift `is_selected` from GoatData → ActorData so HerdComponent works on any subclass (Patch 2)

### Phase 3 (Visual Polymorphism)

- [x] ✅ `ActorVisualComponent.setup_visuals()` polymorphic dispatch on ActorData (completed in Extended Slice / patch 1)
- [ ] ⬜ Add `if data is GoblinData` / `FarmerData` / `ElementalData` branches to `setup_visuals()` (Patch 3 follow-up — uses the data resources created in Patch 2)
- [ ] ⬜ Per-type renderer scenes (currently only `GoatRenderer` exists for 2D cards) — gated on `GoatRenderer` rename
- [ ] ⬜ (Optional) Add VisualConfig resource

### Phase 4 (Signals & Events)

- [x] ✅ Update GameEvents signals to be actor-agnostic (`actor_selection_toggled(actor: ActorData, is_selected: bool)` — completed in Extended Slice)
- [x] ✅ Update ItemsAutoload.selected_goat → selected_actor_data (completed in Extended Slice)

### Phase 5 (Spawning & Factory)

- [x] ✅ Create ActorFactory pattern (completed in Extended Slice)
- [x] ✅ Refactor ArenaSpawner to use ActorFactory (completed in Extended Slice)

### Patch 2 Known Gaps (queued for Patch 3)

- [ ] ⬜ `ProgressionComponent.advance_day()` (Core/Managers/ProgressionComponent.gd:12-23) reads GoatData-only fields (`age_days`, `stamina_max`, `stamina_current`). Adding a non-goat to the herd via breeding will throw on the next day advance. Lift these fields to ActorData OR gate on subclass type.
- [ ] ⬜ `HerdManager.sell_goat()` (Components/BreedingComponents/HerdManager.gd:84-89) casts to GoatData for `gold_value`. Non-goat sales return 0 gold. Lift `gold_value` to ActorData OR add a per-subclass override.
- [ ] ⬜ `GoatActor.die()` removes self from herd via `HerdManager.remove_goat()`. Mirror this hook on other Actor subclasses so non-goat herd members are cleaned up on death.
- [ ] ⬜ Save round-trip must be smoke-tested in editor once GoblinData/FarmerData/ElementalData scripts are imported. See AGENT_2_PATCH2_LOG.md §9.

### Future (Post-Refactor)

- [ ] ⬜ Implement TraitInheritanceEngine
- [x] ✅ Create GoblinData with traits (Patch 2 — minimal genetic: tribe_color, ear_type)
- [x] ✅ Create ElementalData with element_subtype + glow_intensity (Patch 2 — combination table for fire+water → steam deferred)
