...
VERSION LOG:
  ...
  v1.2 (2026-05-16) - ActorData lifted `is_selected` from GoatData;
                       added `get_display_name()`, `get_info_line()`,
                       `set_display_name()` virtuals so UI cards can render
                       any ActorData subclass polymorphically.
================================================================================
-->
...
Base resource for actor data. Carries ability scores (str/dex/con/int/wis/cha),
breeding state (gender, is_pregnant, pregnancy_timer, pregnancy_father,
is_exhausted, **is_selected**), and visual fields (base_color, pattern_color).
Each `@export` field has a setter that emits `stats_changed` so the UI re-renders.

| Field | Type | Purpose |
|-------|------|---------|
| `is_selected` | bool | Arena selection. **Lifted from GoatData in v1.2** so any ActorData subclass can be selected by `HerdComponent.toggle_selection()` |

**Virtual methods (override in subclasses):**

| Virtual | Default | Override pattern |
|---------|---------|------------------|
| `create_offspring(partner: ActorData) -> ActorData` | `push_error`, returns null | Mix the partner's genes into a same-class kid. See `GoatData.create_offspring()` for the reference algorithm. |
| `get_actor_type() -> String` | Returns class name (`"ActorData"`) | Return canonical lowercase key matching `Actor.element_type` (e.g. `"goat"`, `"goblin"`, `"fire"`). |
| `get_display_name() -> String` | Returns `get_actor_type()` | Return identity field (e.g. `goat_name`, `goblin_name`). |
| `get_info_line() -> String` | Returns gender string (`"Female"` / `"Male"`) | Return a richer one-liner (e.g. GoatData returns `"Lvl: %d - Age: %d"`). |
| `set_display_name(new_name: String) -> void` | No-op | Write through to identity field. Used by `ActorCard`'s name-edit field. |

**Subclasses currently in tree:** `GoatData`, `MimicData`, `GoblinData`, `FarmerData`, `ElementalData` (the last three new in v1.2).
...
| `target: Node3D` | Node3D | Current target actor |
...
add_child(actor)
actor.global_position = Vector3(100, 0, 200)
```
...
| 1.2 | 2026-05-16 | Lifted `is_selected` from GoatData → ActorData; added `get_display_name()`, `get_info_line()`, `set_display_name()` polymorphic virtuals so UI cards can render any ActorData subclass. New section "Data Resources" documents the ActorData base + virtual contract. |
