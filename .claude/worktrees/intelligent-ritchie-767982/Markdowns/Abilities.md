...
VERSION LOG:
  ...
  v1.1 (2026-05-16) - Added AbilityRegistry autoload section (Extended Slice)
================================================================================
-->

## Overview

The Ability system provides special maneuvers and reactive abilities for actors. It's designed around three key concepts:
...
3. **AbilityRegistry** (autoload) — Boot-time scan of every `AbilityAction` subclass under `res://Components/ActorComponents/AbilityComponents/`, exposing `get_ability_names()`, `get_ability_script(name)`, and `instantiate_for(name, actor, component)` for safe per-actor cloning

This architecture allows actors to have multiple abilities, each with its own logic, cooldowns, and behaviors — and lets ability-copier creatures (e.g. the Mimic, see `Markdowns/Mimic.md`) instantiate fresh, actor-bound copies without sharing the donor's `RefCounted` instance.
...
### 0. AbilityRegistry (autoload)

**File:** `res://Core/AbilityRegistry.gd`
**Autoload key:** `AbilityRegistry` (registered in `project.godot`)
**Type:** `Node`

Singleton that scans `res://Components/ActorComponents/AbilityComponents/` on
boot and builds a `StringName -> Script` cache keyed by each ability's
`ability_name`. Skips the abstract base files (`AbilityAction.gd`,
`AbilityComponent.gd`); other folder contents are validated by probing
`script.new(temp_actor, temp_component) is AbilityAction` so non-ability
scripts in the folder cannot corrupt the cache.
...
1. Replaces the inline instantiate-and-free reflection in `UI/MainMenu.gd`
   (formerly `_get_goblin_abilities` / `_get_goat_abilities`) with a single
   boot-time scan + cheap dictionary lookups.
2. Powers `Components/ActorComponents/SkillCopyComponent.gd` — when the
   Mimic copies a stolen ability it calls `instantiate_for(name, self,
   ability_component)` so the clone is bound to the Mimic, not to the donor.
   `AbilityAction` is `RefCounted` and stores `actor: Actor` set in `_init`;
   sharing a single instance would cross-wire the field.

**Adding a new ability:** drop a new `AbilityAction` subclass into
`Components/ActorComponents/AbilityComponents/`, set `ability_name` in
`_init`, and restart the editor. The Mimic vocabulary, the UI display path,
and any future copier-style creature automatically pick it up — no manual
