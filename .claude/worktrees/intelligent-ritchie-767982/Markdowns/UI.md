...
VERSION LOG:
  ...
  v1.1 (2026-05-16) - MainMenu ability lookups now route through
                       AbilityRegistry autoload (Extended Slice). Display
                       behavior unchanged; implementation no longer does
                       per-open instantiate-and-free reflection cycles.
  v1.2 (2026-05-16) - ActorCard refactored to take any ActorData subclass.
                       `actor_data: ActorData` is now the primary API;
                       `goat_data: GoatData` is preserved as a typed alias.
                       Card display uses `ActorData.get_display_name()` /
                       `get_info_line()` / `set_display_name()` virtuals
                       so non-goat actors render correctly without subclass
                       casts. Renderer assignment is gated on
                       `actor_data is GoatData` until per-type 2D renderers
                       exist (GoatRenderer rename is deferred).
================================================================================
-->
...
| `_get_goblin_abilities()` / `_get_goat_abilities()` | Per-actor allow-list helpers that filter against `AbilityRegistry` (autoload). Names that the registry does not know about are silently dropped. Replaces the previous instantiate-and-free reflection that ran on every menu open. |
| `_resolve_ability_names_for(allow_list)` | Shared helper used by the two per-actor lookups. Falls back to the literal labels if the autoload is missing so the menu stays readable in stripped builds. |
...
- Generic over `ActorData` — accepts any subclass (Goat, Mimic, Goblin, Farmer, Elemental)
- Also accepts a runtime `Actor` node when no data resource is bound
- Editable name via LineEdit (routes through `ActorData.set_display_name()` virtual)
- Status indicators: `[P]` pregnant, `[E]` exhausted
- 2D card-side renderer (`GoatRenderer`) is currently goat-only; non-goat cards hide the renderer until a per-type renderer system lands

### Properties

| Property | Type | Description |
|----------|------|-------------|
| `actor_data` | `ActorData` | **Primary API.** Setter forwards to `setup()`; getter casts `data_resource` to ActorData |
| `goat_data` | `GoatData` | Legacy typed alias. Setter forwards to `actor_data`; getter casts to GoatData (null for non-goat) |
| `actor_resource` | `ActorData` | Internal back-compat alias; forwards to `actor_data` |

### Key Methods

| Method | Description |
|--------|-------------|
| `set_actor(p_actor)` | Sets runtime actor to display. Now reads `p_actor._data` polymorphically (was: only handled `GoatActor.goat_data`) |
| `setup(p_data)` | Wires `stats_changed` re-render on the bound ActorData |
| `_update_resource_ui()` | Renders name/info/stats via polymorphic virtuals. Gates renderer assignment on `actor_data is GoatData` |
| `_toggle_arena_selection()` | Toggles arena selection via `HerdManager.toggle_selection(actor_data)` (reads `is_selected` on ActorData base) |
