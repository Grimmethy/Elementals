...
## Typed accessor mirroring `GoatActor.goat_data` / `MimicActor.mimic_data`.
## Lets breeding code reach the goblin's data resource without a manual cast.
## When `_data == null` the goblin runs in legacy spawn mode (stats hardcoded
## below). When a GoblinData is assigned, `Actor._on_data_changed()` syncs
## the ability scores BEFORE `_ready()` ever runs the hardcoded fallback.
var goblin_data: GoblinData:
	get: return _data as GoblinData
	set(v): _data = v
...
func die() -> void:
	if is_dead:
		return
	super.die()
	# If a designer assigned a GoblinData (i.e. this goblin is a first-class
	# herd member), keep the persistent herd in sync on death. Legacy spawn
	# paths leave `_data == null` — they were never in the herd, so the
	# autoload lookup and `herd.erase` skip cleanly.
	_remove_from_herd_if_present()

## Mirrors `GoatActor.die()`'s herd-removal hook. Idempotent — safe to call
## twice; `HerdComponent.remove_goat()` does an `Array.erase()` which is a
...
func _ready() -> void:
	...
	# Stat bonuses — only hardcode when no GoblinData was assigned. With a
	# GoblinData, `Actor._on_data_changed()` already populated these from
	# the resource and we must NOT clobber them.
	if _data == null:
