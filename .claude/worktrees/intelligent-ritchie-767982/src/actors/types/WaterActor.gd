...
## Typed accessor mirroring `GoatActor.goat_data`. Optional — water actors
## continue to work with `_data == null` (legacy spawn behaviour). When a
## designer assigns an ElementalData with `element_subtype = &"water"`,
## `Actor._on_data_changed()` syncs ability scores from the resource.
var water_data: ElementalData:
