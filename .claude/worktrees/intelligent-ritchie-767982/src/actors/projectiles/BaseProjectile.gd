...
func _on_body_entered(body: Node3D) -> void:
	...
	# Capture-tool hit path (today: Net). Runs save → stun → catch and
	# consumes the projectile regardless of outcome. We early-return so the
	# damage / _stick / reparent flow below never runs — _stick on a queue_freed
	# actor would push_error in deferred reparent.
	if source_weapon_data and source_weapon_data.is_capture_tool and body is Actor:
		_run_capture_flow(body as Actor)
		queue_free()
		return
...
## Capture-tool hit handler. Lazily attaches a CaptureComponent to the
## target (most wild actors lack one by design — keeps wild actors
## component-light) using the profile referenced by the source weapon.
## Runs the save → stun → catch primitives, prints readouts so the player
## can see outcomes in the Godot console until the Capture UI ships.
##
## The full Capture patch will EXTEND this — add restraint stacking,
## ActorState transitions, drag mechanics — without removing the primitive
## calls below. Keep this function the single hit-side entry point.
func _run_capture_flow(target: Actor) -> void:
	if target.is_dead:
		return
	var profile: CaptureProfile = _load_capture_profile()
	if profile == null:
		print("[Capture] Net hit %s but no profile loaded — no capture attempt." % target.name)
		return
	var capture_comp: CaptureComponent = target.get_node_or_null("CaptureComponent") as CaptureComponent
	if capture_comp == null:
		capture_comp = CaptureComponent.new()
		capture_comp.name = "CaptureComponent"
		capture_comp.capture_profile = profile
		target.add_child(capture_comp)
		capture_comp.setup(target)
	else:
		# Existing component (e.g. designer-attached) — swap profile to the
		# tool's so this hit uses the Net's parameters, not the actor's
		# default. Future restraint-stacking will replace this with merge logic.
		capture_comp.capture_profile = profile

	if not capture_comp.can_be_captured():
		print("[Capture] Net hit %s but target cannot be captured (allowlist / state)." % target.name)
		return

	# Primitive 1: save roll. SUCCESS = nothing happens.
	var saved: bool = capture_comp.roll_save()
	if saved:
		return

	# Primitive 2: stun (the "Restrained" stand-in for v1.0).
	capture_comp.apply_stun(profile.stun_duration)

	# Primitive 3: catch roll. SUCCESS = capture; FAIL = stay stunned, escape.
	var caught: bool = capture_comp.roll_catch()
	if caught:
		capture_comp.apply_capture()

## Loads the CaptureProfile referenced by the source weapon. Returns null
## if the weapon has no path set (falls back to "no profile" branch) — we
## intentionally do NOT build a default profile here so the capture system
## is fully data-driven: a Net entry without a profile path is a soft-fail,
## not a silent default.
func _load_capture_profile() -> CaptureProfile:
	if source_weapon_data == null:
		return null
	var path: String = source_weapon_data.capture_profile_path
	if path.is_empty():
		return null
	var res: Resource = load(path)
	return res as CaptureProfile
