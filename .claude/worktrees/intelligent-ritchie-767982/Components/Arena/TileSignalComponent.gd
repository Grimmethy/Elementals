...
## ID of the registered GameClock tick callback (for the periodic actor rescan).
## -1 if not yet registered.
var _refresh_tick_id: int = -1

func setup(p_arena: Node) -> void:
	arena = p_arena
	_refresh_tracked_actors()

	# Routing rule (AGENTS.md): no Timer.new() — register the periodic actor
	# rescan with the centralized GameClockComponent owned by the arena.
	# The arena creates GameClock before tile_signals (see Arena._setup_components),
	# so this fetch is safe at setup() time. We defer to be extra robust against
	# load-order changes.
	call_deferred("_register_refresh_tick")

func _register_refresh_tick() -> void:
	if _refresh_tick_id != -1:
		return
	var clock: GameClockComponent = null
	if arena and arena.has_method("get_game_clock"):
		clock = arena.get_game_clock()
	if clock:
		# 1.0s cadence matches the previous Timer wait_time. Picks up actors
		# that were added to arena.actors without going through register_actor().
		_refresh_tick_id = clock.register_tick(_refresh_tracked_actors, 1.0)
	else:
		# Last-resort: try again next frame. If the arena still has no clock by
		# then, refreshes are skipped — but actors added through register_actor()
		# (the normal path used by ArenaSpawner) continue to work without rescans.
		call_deferred("_register_refresh_tick")
