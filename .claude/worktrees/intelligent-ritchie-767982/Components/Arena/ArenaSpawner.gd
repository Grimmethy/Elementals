...
## Number of Mimics to seed at arena init. Default 1 — a single Mimic on
## the map is enough to be visible / testable for the Patch 3 acceptance
## flow (throw Net at it, watch save/catch readouts). Designer-tunable.
@export_range(0, 10, 1) var mimic_count: int = 1

## Dispatcher composed in by setup(). Every spawn path that knew about
## per-type `match` arms now routes through the factory's StringName keys.
...
func setup(p_arena: Node3D) -> void:
	arena = p_arena
	_actor_factory = ActorFactory.new()
	_actor_factory.name = "ActorFactory"
	add_child(_actor_factory)
	_register_default_actor_types()

## Pre-registers every actor type the spawner currently supports. New types
## can be added either here or by calling `_actor_factory.register()` later;
## the factory is the single source of truth for "type string -> scene".
func _register_default_actor_types() -> void:
	_actor_factory.register(&"farmer", farmer_scene)
	_actor_factory.register(&"fire", fire_actor_scene)
	_actor_factory.register(&"water", water_actor_scene)
	_actor_factory.register(&"goat", goat_actor_scene)
	_actor_factory.register(&"goblin", goblin_scene)
	_actor_factory.register(&"scarecrow", scarecrow_scene)
	_actor_factory.register(&"mimic", mimic_scene)
...
func spawn_initial_actors() -> void:
	...
	# Patch 3: seed Mimics so the player can actually find one. Goes through
	# the same _get_random_spawn_tile + spawn_actor_at_tile pipeline as
	# goblins, so faction / proximity / spawn-tile-filter rules apply.
	# `mimic_count` defaults to 1 — see export above for designer-tunable.
...
func get_selected_actor_scene() -> PackedScene:
	...
	# Route through the factory so any future actor type registered at runtime
	# is reachable from the main menu's "Play as" selection without editing
	# this method.
	if _actor_factory:
		var scene: PackedScene = _actor_factory.get_scene(StringName(type))
		if scene:
			return scene
	return farmer_scene
...
func spawn_actor_at_tile(type: String, tile: HexTileData) -> Node3D:
	# Dispatch via ActorFactory — preserves the old call signature and the
	# return type (Node3D) while letting any registered actor type spawn here.
	if not _actor_factory:
		return null
	var spawn_pos: Vector3 = tile.position + Vector3(0, arena._get_tile_surface_y(tile) + 1.0, 0)
	var actor: Actor = _actor_factory.spawn(StringName(type), arena, spawn_pos)
	if actor == null:
		return null
	arena.actors.append(actor)
	if arena.get("tile_signals"):
