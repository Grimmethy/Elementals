...
## Generic visual-dispatch entry point used by polymorphic callers (Mimic
## morph, future ActorCard renderer, etc.). Dispatches on `data`'s class so
## adding a new ActorData subclass only requires an additional branch here.
## A null `data` is a documented no-op so callers can blindly hand off
## `target._data` even when the target carries no resource (Goblin, Farmer).
##
## Backwards compatibility: `update_goat_visuals()` continues to work, and the
## existing direct callers (GoatActor._on_data_changed_impl, _setup_goat_visuals
## above) are not touched.
func setup_visuals(data: ActorData) -> void:
	if data == null:
		return
	if data is GoatData:
		update_goat_visuals(data)
	elif data is MimicData:
		_apply_mimic_visuals(data)
	# Other actor types: intentional no-op until a per-type visual exists.

## Mimic visual is intentionally minimal in v1.0:
## - Tints the body sprite/mesh to the mimic's `base_color`.
## - Resizes the body model uniformly if one is present.
## A future patch can swap in mesh morphs once the procedural character
## framework lands.
func _apply_mimic_visuals(mimic_data: MimicData) -> void:
	if not body:
		return
	if body_sprite:
		# Sprite3D body — modulate to the mimic's signature color.
		body_sprite.modulate = mimic_data.base_color
	elif body_model:
		var scale_factor: float = 0.7
		body_model.scale = Vector3.ONE * scale_factor
		initial_model_scale = body_model.scale
		# Try to tint a shared-material body mesh if the model exposes one.
		...
		if combiner:
			...
			if body_mesh and body_mesh.material is ShaderMaterial:
				...
				mat.set_shader_parameter("albedo", mimic_data.base_color)
