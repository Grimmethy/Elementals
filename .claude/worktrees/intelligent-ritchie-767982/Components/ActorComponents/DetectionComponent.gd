...
## ID of the registered GameClock tick callback for perception expiry.
## -1 when not registered. Replaces the old per-actor Timer node (AGENTS.md
## "no Timer nodes on actors" rule). Always-on at EXPIRY_TICK_INTERVAL while
## the component lives; the callback no-ops when perceived_enemies is empty.
var _expiry_tick_id: int = -1

## Fixed cadence at which the expiry callback runs (seconds). Replaces the
## dynamic wait_time pattern of the previous one-shot Timer — perceived
## actors are decremented by exactly this delta each tick, so coarser values
## reduce CPU at the cost of expiry granularity.
const EXPIRY_TICK_INTERVAL: float = 0.5
...
func setup(p_actor: Node3D) -> void:
	...
	# Register perception-expiry callback with the centralized GameClock
	# (via Actor.register_tick — falls back to local tick system if no clock).
	_setup_expiry_tick()
...
func _setup_expiry_tick() -> void:
	# Register an always-on tick at EXPIRY_TICK_INTERVAL. Cheaper than a Timer
	# node and routed through the centralized GameClock per AGENTS.md.
	if not actor or _expiry_tick_id != -1:
		return
	_expiry_tick_id = actor.register_tick(_on_expiry_tick, EXPIRY_TICK_INTERVAL)

## Replacement for the old dynamic-wait Timer. Called every EXPIRY_TICK_INTERVAL
## seconds; decrements every perceived-enemy timer by that interval and emits
## `actor_lost` for any that hit zero. Skips work fast when nothing is perceived.
func _on_expiry_tick() -> void:
	if perceived_enemies.is_empty():
		return

	var expired: Array[Node3D] = []
	for enemy in perceived_enemies.keys():
		perceived_enemies[enemy] -= EXPIRY_TICK_INTERVAL
...
## Legacy helper retained as a no-op so any external caller still compiles.
## With the GameClock-driven _on_expiry_tick the schedule is implicit.
...
func _notification(what: int) -> void:
	if what == Node.NOTIFICATION_EXIT_TREE:
		_cached_arena = null
		# Unregister the GameClock tick so it doesn't outlive the actor.
		if _expiry_tick_id != -1 and is_instance_valid(actor):
			actor.unregister_tick(_expiry_tick_id)
			_expiry_tick_id = -1
