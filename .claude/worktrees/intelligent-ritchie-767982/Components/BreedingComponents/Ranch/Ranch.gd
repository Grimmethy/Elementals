...
# Typed as ActorData so the Ranch can pair any breeding species in the
# future. Today only goats reach the Ranch UI via the herd, but the
# breeding pipeline (BreedingComponent.breed, HerdManager.next_day) is
# already actor-agnostic. The cheat panel still builds a GoatData
# specifically — see `_on_max_level_goat_pressed` below.
...
func refresh_ui() -> void:
	...
	# Sort herd by display name for stability. Uses the polymorphic virtual
	# so non-goat species also sort cleanly.
	var sorted_herd = HerdManager.herd.duplicate()
	sorted_herd.sort_custom(func(a, b):
		return a.get_display_name() < b.get_display_name()
	)

	for actor in sorted_herd:
		var card = preload("res://UI/DisplayCard/ActorCard.tscn").instantiate()
		# Use the generic `actor_data` setter so any ActorData subclass
		# flows through. The `goat_data` alias on ActorCard has a typed
		# `GoatData` setter that would reject non-goat values.
		card.actor_data = actor
		card.selected.connect(_on_actor_selected)

		if actor.gender == ActorData.Gender.FEMALE:
...
func _on_actor_selected(actor: ActorData) -> void:
	if actor.gender == ActorData.Gender.FEMALE:
		if selected_doe == actor:
			selected_doe = null
		else:
			selected_doe = actor
	else:
		if selected_buck == actor:
			selected_buck = null
		else:
			selected_buck = actor
	refresh_ui()
...
func _update_breeding_selection() -> void:
	if selected_doe:
		var card = preload("res://UI/DisplayCard/ActorCard.tscn").instantiate()
		card.actor_data = selected_doe
		selected_doe_card.add_child(card)

	if selected_buck:
		var card = preload("res://UI/DisplayCard/ActorCard.tscn").instantiate()
		card.actor_data = selected_buck
