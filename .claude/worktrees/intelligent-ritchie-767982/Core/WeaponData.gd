...
## True if this weapon initiates a capture attempt on hit instead of dealing
## damage. Today parsed from the notes string keyword `"capture"` — see
## `_parse_notes()`. Consumed by `BaseProjectile._on_body_entered`, which
## lazily attaches a `CaptureComponent` to the target using the profile
## referenced by `capture_profile_path`. Designer-tunable via the inspector.
@export var is_capture_tool: bool = false
## Path to a `CaptureProfile` `.tres` describing save DC / stun duration /
## catch chance for this weapon. Only meaningful when `is_capture_tool`.
## Empty string = use built-in defaults at hit time. Not parsed from notes
## (paths don't belong in note strings) — set per-entry in
## `Core/ItemsAutoload.gd` after `_add(...)`.
@export_file("*.tres") var capture_profile_path: String = ""
...
func _parse_notes() -> void:
	...
	is_heavy = "heavy" in n
	# Flag this weapon as a capture tool when the notes contain the literal
	# token "capture" — designer-readable, no string-matching on the weapon
	# name itself. The actual save DC / stun / catch chance live in a
	# separate `CaptureProfile.tres` referenced via `capture_profile_path`.
	is_capture_tool = "capture" in n
