...
## Generic selection bus for any ActorData subclass (GoatData, MimicData, etc.).
## Introduced in the Extended Slice (B8) as the forward-looking replacement for
## the goat-specific `selected_goat` field. The legacy `selected_goat` property
## below is a deprecated alias that proxies to this field so the existing
## goat-only flow continues to work unchanged.
var selected_actor_data: ActorData

## DEPRECATED — use `selected_actor_data` for new code.
## Kept as a typed alias to preserve the goat-only flow. Reads return the
## current `selected_actor_data` cast to GoatData (null for non-goats). Writes
## go straight to `selected_actor_data` without re-emitting `goat_selected`;
## use `set_selected_goat()` if you need that signal explicitly.
var selected_goat: GoatData:
	get:
		return selected_actor_data as GoatData
	set(v):
		selected_actor_data = v
...
func _init_weapons() -> void:
	...
	# Net: thrown capture tool. The "capture" token in notes flips
	# WeaponData.is_capture_tool true via _parse_notes(). The profile path
	# is set as a separate field below because file paths don't belong in
	# notes strings. On hit, BaseProjectile lazily attaches a
	# CaptureComponent to the target using this profile.
	_add("Net", "1 gp", "-", "none", 3, "Thrown (range 5/15), capture", net_icon)
	for w in weapons:
		if w.name == "Net":
...
func set_selected_goat(goat: GoatData) -> void:
	# Routes through the generic bus so any non-goat consumer that watches
	# `selected_actor_data` stays in sync, then emits the legacy goat signal.
	selected_actor_data = goat
	goat_selected.emit(goat)

func set_selected_actor(actor: Actor) -> void:
	selected_actor = actor
	actor_selected.emit(actor)
	# Surface the actor's data on the generic bus regardless of subclass.
	# Goat handling stays as it was so the legacy `goat_selected` listeners
	# (Ranch UI, save flow) keep firing without change. CAREFUL: for the
	# non-goat path we must NOT route through `set_selected_goat(null)` —
	# that clobbers `selected_actor_data` back to null after we just wrote
	# the actor's data above. Emit the legacy null signal inline instead.
	if actor and "_data" in actor:
		selected_actor_data = actor._data
	else:
		selected_actor_data = null
	if actor and actor is GoatActor:
		set_selected_goat(actor.goat_data)
	else:
		goat_selected.emit(null)
