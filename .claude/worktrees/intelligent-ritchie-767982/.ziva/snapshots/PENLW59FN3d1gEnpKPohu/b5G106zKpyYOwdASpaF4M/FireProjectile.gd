...
func _apply_effect_to_tile() -> void:
	...
	if tile.current_state == HexTile.State.STONE:
		queue_free()
		return

	_last_tile = tile
	var delta = global_transform.origin - tile.global_transform.origin
	delta.y = 0
	if delta.length_squared() > _effect_range * _effect_range:
		return
	if tile.apply_fire():
