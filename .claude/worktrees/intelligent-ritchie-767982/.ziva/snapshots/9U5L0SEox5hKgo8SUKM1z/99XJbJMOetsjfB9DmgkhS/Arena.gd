...
var current_controlled_elemental: Elemental:
	set(value):
		...
		if current_controlled_elemental:
			...
			if reticle:
				var c = current_controlled_elemental.get_elemental_color()
				reticle.color = c
...
func _setup_reticle() -> void:
	...
	reticle = ColorRect.new()
	reticle.size = Vector2(4, 4)
	reticle.pivot_offset = Vector2(2, 2)
	reticle.mouse_filter = Control.MOUSE_FILTER_IGNORE
	canvas_layer.add_child(reticle)
	
	var directions = [Vector2(0, -10), Vector2(0, 10), Vector2(-10, 0), Vector2(10, 0)]
	var sizes = [Vector2(2, 8), Vector2(2, 8), Vector2(8, 2), Vector2(8, 2)]
	var offsets = [Vector2(-1, -8), Vector2(-1, 0), Vector2(-8, -1), Vector2(0, -1)]
	
	for i in range(4):
		var rect = ColorRect.new()
		rect.size = sizes[i]
		rect.position = directions[i] + offsets[i]
...
func _process(delta: float) -> void:
	...
	if reticle:
		reticle.position = get_viewport().get_mouse_position() - reticle.size / 2.0
