...
func _on_elemental_died(e: Node3D) -> void:
	# Handled internally if needed, but the main check is for player-owned goats
	if elementals.has(e):
		# If it's a goat, we erase it, otherwise we keep it as it's just respawning
...
func _select_initial_elemental() -> void:
	...
	# Prefer selecting a goat if one exists
