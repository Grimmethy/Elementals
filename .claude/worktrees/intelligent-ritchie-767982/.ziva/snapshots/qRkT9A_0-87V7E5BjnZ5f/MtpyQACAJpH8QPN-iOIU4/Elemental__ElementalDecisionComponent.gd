...
func _get_tree_avoidance_vector() -> Vector3:
	...
	# Check a small radius around the elemental for trees
	var nearby_tiles = elemental._arena_grid.get_tiles_within_distance(elemental.global_position, 2.5)
	for tile in nearby_tiles:
		if tile.feature and tile.feature is TreeFeature:
			# Pinecones are small, don't avoid them as much as trees/stumps
			if tile.feature.current_state == TreeFeature.State.PINECONE:
				continue
			...
			# Avoidance threshold: trees are roughly hex-sized (1.5)
			if dist < 1.4 and dist > 0.1:
				# Repulsion force gets stronger the closer we are
				avoidance += diff.normalized() * (1.4 - dist)
