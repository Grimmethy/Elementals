...
func _launch_spread_shot(target_position: Vector3) -> void:
	...
	for angle in angles:
		...
		# Spread shots are slightly weaker per projectile but 5 of them
		_do_launch_projectile_with_params(projectile_scene, spawn_position, direction, projectile_speed, 3, projectile_lifetime)
...
func _launch_lob_shot(target_position: Vector3) -> void:
	...
	for i in range(5):
		...
		if projectile.has_method("initialize_lob"):
			projectile.initialize_lob(_arena_grid, spawn_position, cluster_target, projectile_speed * 0.8, 3, projectile_lifetime)
		elif projectile.has_method("initialize"):
			var dir = (cluster_target - spawn_position).normalized()
			projectile.initialize(_arena_grid, spawn_position, 0.0, dir, projectile_speed, 3, projectile_lifetime)
...
func _do_launch_projectile_with_params(scene: PackedScene, spawn_pos: Vector3, direction: Vector3, velocity: float, charges: int, p_lifetime: float) -> void:
	var projectile = scene.instantiate()
	var parent = get_parent() if get_parent() else get_tree().get_current_scene()
	parent.add_child(projectile)
	projectile.global_transform = Transform3D(Basis(), spawn_pos)
	if projectile.has_method("initialize"):
		projectile.initialize(_arena_grid, spawn_pos, _effective_range_world(), direction, velocity, charges, p_lifetime, _projectile_max_range_world())
