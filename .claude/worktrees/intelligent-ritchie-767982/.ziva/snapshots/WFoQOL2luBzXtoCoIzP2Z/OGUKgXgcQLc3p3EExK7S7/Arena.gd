...
func _setup_minimap() -> void:
	# Wait for a frame to ensure the world and nodes are fully initialized.
	await get_tree().process_frame
	var minimap_viewport = get_node_or_null("UI/MinimapContainer/SubViewport")
	if minimap_viewport:
		# In Godot 4, to show the main world in a SubViewport, we can assign the world_3d.
		# Setting own_world_3d to true before assignment can help in some cases.
		minimap_viewport.own_world_3d = true
		minimap_viewport.world_3d = get_world_3d()
		
		# Ensure the minimap camera is correctly positioned and made current for this viewport.
		var camera = minimap_viewport.get_node_or_null("MinimapCamera")
		if camera:
			camera.position = Vector3(28.0, 50.0, 24.0)
...
func _setup_reticle() -> void:
	...
	reticle = Control.new()
	reticle.set_script(preload("res://Reticle.gd"))
...
func _update_debug_info() -> void:
	var debug_checkbox = get_node_or_null("UI/DebugContainer/DebugCheckBox") as CheckBox
	var debug_label = get_node_or_null("UI/DebugContainer/TileCountLabel") as Label
	if not debug_checkbox or not debug_label:
		return
