...
const WHACK_SOUND = preload("res://assets/SoundFiles/Whack.mp3")
...
func _play_whack_effect() -> void:
	...
	# THWAK popup
	var popup = Sprite3D.new()
	popup.texture = THWAK_TEXTURE
	popup.billboard = BaseMaterial3D.BILLBOARD_ENABLED
	popup.pixel_size = 0.02
	add_child(popup)
	popup.position = sprite.position + Vector3(0, 1.0, 0)
	
	var tween = create_tween()
	popup.scale = Vector3.ZERO
	tween.tween_property(popup, "scale", Vector3.ONE * 1.5, 0.1)
	tween.tween_property(popup, "position:y", popup.position.y + 0.5, 0.3).set_delay(0.1)
	tween.parallel().tween_property(popup, "modulate:a", 0.0, 0.3).set_delay(0.1)
