...
func _update_fire_effect(tile: HexTileData, active: bool) -> void:
	if active:
		...
		p.position = tile.position + Vector3(0, 0.5, 0)
		p.emitting = true
...
func _get_fire_particle_from_pool() -> GPUParticles3D:
	...
	Elemental.setup_gpu_particles(p, {
		...
		"scale_min": 0.3 * tile_scale,
		"scale_max": 0.6 * tile_scale,
...
func _setup_physics() -> void:
	...
	var radius = max(w_total * SQRT3 * hex_size, h_total * 1.5 * hex_size) * 0.55 * tile_scale
	...
	floor_col.position = Vector3(center_x, -0.4, center_z) # Surface at 0.1 to match hex tops
	floor_static_body.add_child(floor_col)
	
	# 2. Individual Boundary Walls (Primitives for stability)
	# Using primitive CylinderShape3D instead of mesh-derived concave shapes
	var wall_radius = 1.0 * tile_scale * 0.95 # Slightly smaller than visual to avoid friction sticking
	for tile in tile_data_grid:
		if tile.current_state == TileConstants.State.STONE:
			var wall_shape = CylinderShape3D.new()
			wall_shape.radius = wall_radius
			wall_shape.height = 2.0 # Standard stone tile height
			
			var wall_col = CollisionShape3D.new()
			wall_col.shape = wall_shape
			# Tile origin for stone is at 0.9 (visual height 2.0, top at 1.9, bottom at -0.1)
			wall_col.position = tile.position + Vector3(0, 0.9, 0)
			floor_static_body.add_child(wall_col)
