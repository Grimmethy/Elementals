...
const TEXTURES = {
	...
	State.STUMP: preload("res://assets/generated/stump_simple_frame_0_1774919981.png"),
	State.BURNT_STUMP: preload("res://assets/generated/burnt_stump_simple_frame_0_1774919979.png")
}
...
func take_damage(amount: float, is_fire: bool) -> void:
	if current_state == State.BURNT_STUMP: return # Already "dead" for damage purposes
	
	# Special case: Pinecones and Saplings burn instantly when fire hits them directly via apply_fire?
	# User says "pinecones and saplings when burnt will be removed from play" earlier.
	# Now says "Trees should have 5 hp... Fire should consume 1 hp/sec".
	# I'll assume Pinecones and Saplings are "trees" for the health logic or keep their instant burn.
	# Actually, usually "Tree" implies the fully grown state, but "stump" is also mentioned.
	# I'll apply the HP logic to TREE, STUMP, BURNT_STUMP.
	# Pinecones and Saplings I'll keep as "instant death by fire" or 1HP?
	# Let's give everything 5HP for consistency, but maybe Pinecones have 1HP?
	# The user said "Trees should have 5 hp".
...
func _show_health() -> void:
	if health_label:
		...
		# Hide health after 2 seconds of no damage
		get_tree().create_timer(2.0).timeout.connect(func(): if health >= MAX_HEALTH or is_queued_for_deletion(): health_label.visible = false)
	...
	match element:
		...
		"headbutt":
			if current_state == State.PINECONE:
				...
				return true
			elif current_state == State.TREE or current_state == State.STUMP:
				take_damage(1.0, false)
				return true
	return false

func apply_fire() -> bool:
	# This is called from HexTile _process if the tile is on fire.
	# We already handle fire damage in _process now.
	# But for instant destruction of pinecones/saplings:
	match current_state:
		State.PINECONE, State.SAPLING:
