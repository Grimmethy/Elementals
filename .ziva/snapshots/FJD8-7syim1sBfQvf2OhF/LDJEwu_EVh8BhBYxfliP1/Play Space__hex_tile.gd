...
func _process(delta: float) -> void:
	...
	if tile_type == Type.FIRE:
		...
		# Spread fire after 4 seconds to a random neighbor
...
func apply_element(element: String, direction: Vector3 = Vector3.ZERO) -> bool:
	# Check for environmental features first
...
func apply_water() -> bool:
	if tile_type == Type.FIRE:
		current_state = State.DIRT # Changed from STUMP
		fire_spread_triggered = false
		fire_duration = 0.0
		return true
