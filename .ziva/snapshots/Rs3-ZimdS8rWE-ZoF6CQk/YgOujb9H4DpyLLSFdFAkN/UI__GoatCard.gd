...
func _ready() -> void:
	...
	select_button.text = "Select for Arena"
	select_button.focus_mode = Control.FOCUS_NONE
	select_button.pressed.connect(_on_select_pressed)
	$VBoxContainer.add_child(select_button)
...
func _update_ui() -> void:
	...
	var sel_str = " [SELECTED]" if goat_data.is_selected else ""
	
	stats_label.text = "%s - Age: %d%s%s%s\nStrength: %.1f Toughness: %.1f Speed: %.1f" % [
		gender_str, goat_data.age_days, preg_str, exh_str, sel_str,
		goat_data.strength, goat_data.toughness, goat_data.speed
	]
	
	if select_button:
		select_button.text = "Deselect" if goat_data.is_selected else "Select for Arena"
		select_button.disabled = goat_data.is_exhausted and not goat_data.is_selected
