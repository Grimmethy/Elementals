...
func _setup_elemental() -> void:
	# Virtual method for subclasses to configure particles, etc.
	pass
...
func _handle_controlled_movement(delta: float) -> void:
	...
	# Since the camera is looking down from an angle, we need to adjust the direction.
	# Actually, assuming a top-down-ish view where Z is forward.
	...
	if not is_on_floor():
		...
		velocity.y = 0.0
		if Input.is_action_just_pressed("ui_accept"): # space is usually ui_accept
