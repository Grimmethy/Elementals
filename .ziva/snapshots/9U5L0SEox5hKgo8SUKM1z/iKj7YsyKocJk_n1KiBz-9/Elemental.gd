...
func take_damage(amount: int) -> void:
	current_hp -= amount
	print(name, " took ", amount, " damage. Current HP: ", current_hp)
	if current_hp <= 0:
		die()

func die() -> void:
	print(name, " has died!")
	# For now, let's respawn them at their origin
	current_hp = max_hp
...
func _apply_ground_effects() -> void:
	_update_tile_below()
	if _ground_tile:
		_check_tile_damage(_ground_tile)
		_do_tile_effect(_ground_tile)

func _check_tile_damage(tile: HexTile) -> void:
