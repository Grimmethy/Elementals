...
func _spawn_elementals() -> void:
	...
	if has_node("/root/GoatManager"):
		...
		if not gm.herd_updated.is_connected(_sync_herd_to_arena):
			gm.herd_updated.connect(_sync_herd_to_arena)
