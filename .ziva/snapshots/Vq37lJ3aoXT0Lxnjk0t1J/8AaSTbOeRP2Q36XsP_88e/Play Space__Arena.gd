	...
	for y in total_h:
		for x in total_w:
			...
			if state == TileConstants.State.GRASS and randf() < 0.05:
				...
				add_child(tree)
				tree.transform.origin = pos + Vector3(0, h_val * height_step, 0)
				tile.feature = tree
...
func _update_fire_effect(tile: HexTileData, active: bool) -> void:
	if active:
		...
		var h_offset = tile.height_level * height_step
...
func _get_tile_transform(tile: HexTileData) -> Transform3D:
	var t = Transform3D()
	var h_offset = tile.height_level * height_step
	t.origin = tile.position + Vector3(0, h_offset, 0)
...
func _setup_physics() -> void:
	...
	# Using primitive CylinderShape3D for each tile to handle height levels
...
func _spawn_type(type: String, x: int = -1, y: int = -1) -> void:
	...
	var h_offset = 0.0
	if tile:
		h_offset = tile.height_level * height_step
