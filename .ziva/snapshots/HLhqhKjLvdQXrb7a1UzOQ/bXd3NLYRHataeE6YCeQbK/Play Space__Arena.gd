	...
	for state in TileConstants.State.values():
		...
		match state:
			TileConstants.State.GRASS:
				mat.set_shader_parameter("edge_noise_amplitude", 0.18)
				mat.set_shader_parameter("edge_noise_frequency", 12.0)
				mat.set_shader_parameter("edge_noise_bias", 0.1) # Pull grass down further
			TileConstants.State.DIRT, TileConstants.State.MUD:
				mat.set_shader_parameter("edge_noise_amplitude", 0.12)
				mat.set_shader_parameter("edge_noise_frequency", 6.0)
				mat.set_shader_parameter("edge_noise_bias", -0.05) # Slightly retracted
			TileConstants.State.STONE:
				mat.set_shader_parameter("edge_noise_amplitude", 0.05)
				mat.set_shader_parameter("edge_noise_frequency", 4.0)
			TileConstants.State.PUDDLE:
				mat.set_shader_parameter("edge_noise_amplitude", 0.08)
				mat.set_shader_parameter("edge_noise_frequency", 8.0)
				mat.set_shader_parameter("rim_highlight", 0.4) # Shiny water edge
