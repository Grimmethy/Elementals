...
@onready var _ranch_button: Button = get_node_or_null("UI/RanchButton")
@onready var _ranch_ui: RanchUI = get_node_or_null("UI/RanchUI")
...
func _select_initial_elemental() -> void:
	...
	for i in range(elementals.size()):
		var e = elementals[i]
		if e.name.to_lower().contains(target_type):
...
func _setup_ui_connections() -> void:
	...
	if _ranch_button and _ranch_ui:
		_ranch_button.focus_mode = Control.FOCUS_NONE
		_ranch_button.pressed.connect(_ranch_ui.toggle)
...
func _spawn_elementals() -> void:
	...
	# Populate Goats from GoatManager
	_sync_herd_to_arena()
	
	# Connect to herd updates for future births
	if not GoatManager.herd_updated.is_connected(_sync_herd_to_arena):
		GoatManager.herd_updated.connect(_sync_herd_to_arena)

func _sync_herd_to_arena() -> void:
	# Find existing goat elementals
	var existing_goats = {} # GoatData -> GoatElemental
	for e in elementals:
		if e is GoatElemental and e.goat_data:
			existing_goats[e.goat_data] = e
	
	# Add missing goats
	for goat_data in GoatManager.herd:
		if not existing_goats.has(goat_data):
			_spawn_goat_from_data(goat_data)
	
	# Cleanup removed goats (optional, if you sell a goat)
	var herd_set = {}
	for g in GoatManager.herd: herd_set[g] = true
	
	var to_remove = []
	for goat_data in existing_goats:
		if not herd_set.has(goat_data):
			to_remove.append(existing_goats[goat_data])
...
func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed("ui_focus_next"): # Tab key usually
		if _ranch_ui: _ranch_ui.toggle()
		return
