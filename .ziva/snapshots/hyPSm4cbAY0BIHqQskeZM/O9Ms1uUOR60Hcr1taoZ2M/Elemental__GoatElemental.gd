...
func _on_goat_data_changed() -> void:
	...
	match goat_data.body_type:
		GoatData.BodyType.SMALL: s = 0.12
		GoatData.BodyType.MEDIUM: s = 0.15
		GoatData.BodyType.LARGE: s = 0.18
...
func _update_goat_visuals() -> void:
	...
	# For 3D, we'll just hardcode them or use a constant here for simplicity
	...
	if _body:
		...
		_body.modulate = goat_data.base_color
